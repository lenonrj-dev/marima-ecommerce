...
type ListMeReviewsInput = { page: number; limit: number };
...
function getStoreOrderBy(sort: StoreReviewSort): Prisma.ReviewOrderByWithRelationInput[] {
  if (sort === "oldest") return [{ createdAt: "asc" }];
  if (sort === "rating_desc") return [{ rating: "desc" }, { createdAt: "desc" }];
  if (sort === "rating_asc") return [{ rating: "asc" }, { createdAt: "desc" }];
...
async function resolveProductIdentity(input: string): Promise<ProductIdentity> {
  ...
  if (!raw) throw new ApiError(400, "Produto inválido.");
...
async function getPublishedSummaryByProductId(productId: string): Promise<ProductReviewSummary> {
  ...
  for (const row of groupedByRating) {
    ...
    if (rating >= 1 && rating <= 5) {
      distribution[rating as 1 | 2 | 3 | 4 | 5] = Math.max(0, Number(row._count._all || 0));
    }
  }
...
export async function listPublishedProductReviews(input: {
  ...
  const where: Prisma.ReviewWhereInput = { productId: product.id, status: "published" };
...
export async function createMeReview(customerId: string, input: CreateMeReviewInput) {
  ...
  if (comment.length < 5 || comment.length > 2000) {
    throw new ApiError(400, "Comentário deve ter entre 5 e 2000 caracteres.");
  }
  ...
  if (existing) throw new ApiError(409, "Você já avaliou este produto.");
  ...
  try {
    ...
    if (error?.code === "P2002") throw new ApiError(409, "Você já avaliou este produto.");
    throw error;
  }
...
export async function patchAdminReviewStatus(reviewId: string, status: ReviewStatus) {
  ...
  if (!review) throw new ApiError(404, "Avaliação não encontrada.");
  ...
  if (!refreshed) throw new ApiError(404, "Avaliação não encontrada.");
